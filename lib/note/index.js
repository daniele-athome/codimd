...
async function showNote (req, res) {
  ...
  } else if (!note.ownerId && userId) {
    // note with no owner, first one gets ownership
    logger.info('Giving ownership of note ' + note.id + ' to user ' + userId)
    await Note.update({
      ownerId: userId
    }, {
      where: {
  ...
  if ((note.alias && noteId !== note.alias) || (!note.alias && noteId !== id)) {
    return res.redirect(config.serverURL + '/' + encodeURIComponent(note.alias || id))
  }
...
async function showPublishNote (req, res) {
  ...
  if ((note.alias && shortid !== note.alias) || (!note.alias && shortid !== note.shortid)) {
    return res.redirect(config.serverURL + '/s/' + encodeURIComponent(note.alias || note.shortid))
  }
...
async function noteActions (req, res) {
  ...
  switch (action) {
    ...
    case 'pandoc':
      ...
      break
    default:
      return res.redirect(config.serverURL + '/' + encodeURIComponent(noteId))
  }
}
