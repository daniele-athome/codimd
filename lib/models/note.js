...
module.exports = function (sequelize, DataTypes) {
  var Note = sequelize.define('Note', {
    ...
    hooks: {
        ...
        return new Promise(function (resolve, reject) {
          ...
          if (!note.content) {
            ...
            if (Note.checkFileExist(filePath)) {
              ...
              if (config.debug) { logger.debug('Creating empty note file for: ' + filePath) }
              // create empty file for later
              try {
      ...
      beforeUpdate: function (note, options, callback) {
        if (note.title && note.title !== 'Untitled' && note.alias && note.title !== note.alias) {
          // Note already had an alias, check if title changed and rename file if needed
          const oldFilePath = path.join(config.docsPath, note.alias + '.md')
          const newFilePath = path.join(config.docsPath, note.title + '.md')
          try {
            fs.renameSync(oldFilePath, newFilePath)
          } catch (err) {
            logger.warn(err)
          }
          note.alias = note.title
        }
      },
  ...
  Note.writeNoteToFilesystem = function (note, filePath) {
    try {
      fs.writeFileSync(filePath, note.content ? note.content : '', 'utf8')
      return true
    } catch (err) {
      logger.warn(err)
      return false
