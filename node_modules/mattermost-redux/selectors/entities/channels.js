...
= (0, _reselect.createSelector)(getChannelsInCurrentTeam, _common.getMyChannelMemberships, function (state
/*: GlobalState*/
) {
  var archived
  /*: ?boolean*/
...
/*: Array<Channel>*/
{
  return channels.filter(function (c) {
    return !myMembers.hasOwnProperty(c.id) && c.type === _constants.General.OPEN_CHANNEL && (archived ? true : c.delete_at === 0);
  });
});
exports.getOtherChannels = getOtherChannels;
var getArchivedChannels
/*: (GlobalState) => Array<Channel>*/
...
{
  return channels.filter(function (c) {
    return myMembers.hasOwnProperty(c.id) && c.delete_at !== 0;
  });
});
...
var canManageAnyChannelMembersInCurrentTeam
/*: (GlobalState) => boolean*/
= (0, _reselect.createSelector)(_common.getMyChannelMemberships, _teams.getCurrentTeamId, function (state
/*: GlobalState*/
) {
  return (
    /*: GlobalState*/
...
/*: boolean*/
{
  for (var _i = 0, _Object$keys = Object.keys(members); _i < _Object$keys.length; _i++) {
...
= (0, _helpers.createIdsSelector)(_common.getCurrentUser, _common.getUsers, _users.getUserIdsInChannels, getAllChannels, getUnreadChannelIds, _preferences.getTeammateNameDisplaySetting, function (currentUser, profiles, userIdsInChannels
/*: Object*/
...
/*: (GlobalState) => Array<Channel>*/
= (0, _reselect.createSelector)(_common.getCurrentUser, _common.getUsers, _users.getUserIdsInChannels, getAllChannels, _preferences.getTeammateNameDisplaySetting, function (currentUser
/*: UserProfile*/
, profiles
/*: IDMappedObjects<UserProfile>*/
, userIdsInChannels
...
var getAllActiveChannels = (0, _reselect.createSelector)(getPublicChannels, getPrivateChannels, getDirectChannels, function (publicChannels, privateChannels, directChannels) {
  var allChannels = [].concat(_toConsumableArray(publicChannels), _toConsumableArray(privateChannels), _toConsumableArray(directChannels));
  return allChannels;
});
...
var getDefaultChannelForTeams
/*: (GlobalState) => RelationOneToOne<Team, Channel>*/
= (0, _reselect.createSelector)(getAllChannels, function (channels
/*: IDMappedObjects<Channel>*/
)
/*: RelationOneToOne<Team, Channel>*/
{
  ...
  try {
    for (var _iterator = Object.keys(channels).map(function (key) {
      return channels[key];
...
var getMyFirstChannelForTeams
/*: (GlobalState) => RelationOneToOne<Team, Channel>*/
= (0, _reselect.createSelector)(getAllChannels, _common.getMyChannelMemberships, _teams.getMyTeams, _common.getCurrentUser, function (allChannels
/*: IDMappedObjects<Channel>*/
...
/*: RelationOneToOne<Team, Channel>*/
{
  ...
  try {
    var _loop = function _loop() {
      ...
      // $FlowFixMe
      var teamChannels = Object.values(allChannels).filter(function (channel
      /*: Channel*/
      ) {
        return channel && channel.team_id === team.id && Boolean(myChannelMemberships[channel.id]);
    ...
    for (var _iterator2 = myTeams[Symbol.iterator](), _step2; !(_iteratorNormalCompletion2 = (_step2 = _iterator2.next()).done); _iteratorNormalCompletion2 = true) {
      var _ret = _loop();
...
var getRedirectChannelNameForTeam = function getRedirectChannelNameForTeam(state
/*: GlobalState*/
, teamId
/*: string*/
)
/*: string*/
{
  var defaultChannelForTeam = getDefaultChannelForTeams(state)[teamId];
  var myFirstChannelForTeam = getMyFirstChannelForTeams(state)[teamId];
  var canIJoinPublicChannelsInTeam = !(0, _general.hasNewPermissions)(state) || (0, _roles.haveITeamPermission)(state, {
    team: teamId,
    permission: _constants.Permissions.JOIN_PUBLIC_CHANNELS
  });
  var myChannelMemberships = (0, _common.getMyChannelMemberships)(state);
