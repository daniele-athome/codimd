...
var getCurrentTeamUrl = (0, _reselect.createSelector)(_general.getCurrentUrl, getCurrentTeam, function (state) {
  ...
  var rootURL = "".concat(currentURL || siteURL);
...
var getListableTeamIds = (0, _helpers.createIdsSelector)(getTeams, getTeamMemberships, function (state) {
  return (0, _roles.haveISystemPermission)(state, {
...
}, _general.isCompatibleWithJoinViewTeamPermissions, function (teams, myMembers, canListPublicTeams, canListPrivateTeams, compatibleWithJoinViewTeamPermissions) {
...
var getListableTeams = (0, _reselect.createSelector)(getTeams, getListableTeamIds, function (teams, listableTeamIds) {
  return listableTeamIds.map(function (id) {
    return teams[id];
  });
});
exports.getListableTeams = getListableTeams;
var getSortedListableTeams = (0, _reselect.createSelector)(getTeams, getListableTeamIds, function (state, locale) {
  return locale;
}, function (teams, listableTeamIds, locale) {
  var listableTeams = {};
  ...
  try {
    for (var _iterator = listableTeamIds[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
      var id = _step.value;
...
var getJoinableTeamIds = (0, _helpers.createIdsSelector)(getTeams, getTeamMemberships, function (state) {
  return (0, _roles.haveISystemPermission)(state, {
...
}, _general.isCompatibleWithJoinViewTeamPermissions, function (teams, myMembers, canJoinPublicTeams, canJoinPrivateTeams, compatibleWithJoinViewTeamPermissions) {
  return Object.keys(teams).filter(function (id) {
    var team = teams[id];
    var member = myMembers[id];
...
var getMySortedTeamIds = (0, _helpers.createIdsSelector)(getTeams, getTeamMemberships, function (state, locale) {
  ...
  }).sort((0, _team_utils.sortTeamsWithLocale)(locale)).map(function (t) {
    return t.id;
  });
});
